...
type MergeFlatBody = {
  ...
  emptyTranslationUsesOriginal?: boolean;
};

export async function POST(request: NextRequest) {
  let body: MergeFlatBody;
  try {
    body = (await request.json()) as MergeFlatBody;
  } catch {
    return NextResponse.json({ message: "Request body must be valid JSON." }, { status: 400 });
  }

  const translations = body.translations ?? body.entries ?? {};
  ...
  const backendUrl = new URL("/api/mirror/block-exchange/merge-flat", backendBaseUrl).toString();
  ...
  const backendResponse = await fetch(backendUrl, {
    ...
    body: JSON.stringify({
      ...
      translations,
      entries: {},
      emptyTranslationUsesOriginal: body.emptyTranslationUsesOriginal ?? true
    }),
    cache: "no-store"
  });
  ...
  if (!backendResponse.ok) {
    return NextResponse.json(
      responsePayload ?? { message: "Merge-flat request failed." },
